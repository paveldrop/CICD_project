/*
   ...
   повторяется. -n  Выдает перед каждой строкой ее номер в файле (строки
   нумеруются с 1). -e  example grep -e pattern1 -e pattern2 -e pattern3 grep
   [options] template [file_name] использование regex.h и pcre
*/
...
void UseOptions(char argc, char **argv, FILE *fp, char *pat,
                opt *option_field) {
  regex_t reg[2048];
  char *str = NULL;
  size_t len = 0;
